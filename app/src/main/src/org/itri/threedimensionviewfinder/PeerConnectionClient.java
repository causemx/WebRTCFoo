...
public class PeerConnectionClient {
  ...
  private boolean enableAudio = false;//true;
  @Nullable
  ...
  public static WebRtcListener webRtcListener;
   ...
   */
  public DataChannel.Observer mDataChannelObserver = new DataChannel.Observer() {
    @Override
    ...
    public void onStateChange() {
      Log.d(TAG, "PeerConnectionClient onStateChange:" + (dataChannel != null ? dataChannel.state() : ""));
    }

    @Override
    public void onMessage(DataChannel.Buffer buffer) {
      Log.d(TAG, "PeerConnectionClient onMessage : " + buffer);
      ...
      //TODO  --------------------------------------------------------------------------------------------
      Log.e(TAG, "msg============> : " + msg);
      if (CallActivity.webRtcListener != null) {
        Logging.d(TAG, "PeerConnectionClient onMessage: " + msg);
        CallActivity.webRtcListener.onReceiveDataChannelMessage(msg);
      }
    }
  };
...
//      cameraVideoCapturer.dispose();
  ...
  public void stopVideoCapture(){
    if (videoCapturer instanceof CameraVideoCapturer) {
      if (!isVideoCallEnabled() || isError) {
        Log.e(TAG,
                "Failed to stopVideoCapture camera. Video: " + isVideoCallEnabled() + ". Error : " + isError);
//        return; // No video is sent or only one camera is available or error happened.
      }
      Log.d(TAG, "stopVideoCapture camera");
      CameraVideoCapturer cameraVideoCapturer = (CameraVideoCapturer) videoCapturer;
      cameraVideoCapturer.dispose();
    } else {
      Log.d(TAG, "Will not stopVideoCapture camera, video caputurer is not a camera");
    }
  }
  ...
  public void startVideoCapture() {
    if (videoCapturer instanceof CameraVideoCapturer) {
      if (!isVideoCallEnabled() || isError) {
        Log.e(TAG,
                "Failed to startVideoCapture camera. Video: " + isVideoCallEnabled() + ". Error : " + isError);
        return; // No video is sent or only one camera is available or error happened.
      }
      Log.d(TAG, "startVideoCapture camera");
      CameraVideoCapturer cameraVideoCapturer = (CameraVideoCapturer) videoCapturer;
      cameraVideoCapturer.startCapture(videoWidth, videoHeight, videoFps);
    } else {
      Log.d(TAG, "Will not startVideoCapture camera, video caputurer is not a camera");
    }
  }
